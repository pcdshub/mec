...
def timing_save_t0(): 
    ...
    '''
    spl_vitara_pv.put(spl_vitara.get()) 
    
def timing_getdelay():
    """ get the current delay, based on the  pv"""
    print((-spl_vitara.get()+spl_vitara_pv.get())*1e-9)

def timing(t): 
    '''
    Description : setting a new pump-probe delay. time in seconds
    and compensates with the stage
    '''
    t0_vitara=spl_vitara_pv.get()
    old_vitara_timing=spl_vitara.get()
    new_vitara_timing=t0_vitara-t*1e9
    d=(old_vitara_timing-new_vitara_timing)*1e-9*2.99792458e8*1000/2.0 #motion needed to compensate stage
    #print(d)
    spl_vitara.put(new_vitara_timing)

    #d=(old_vitara_timing-new_vitara_timing)*1e-9*2.99792458e8*1000/2.0 #motion needed to compensate stage
    old_stage_pos=tt.get()
    new_stage_pos=old_stage_pos-d #could be plus d
    #print(new_stage_pos)
    tt.put(new_stage_pos)
