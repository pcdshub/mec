...
class efc:
    ...
    def TraceFormatting(PDTrace, PDFETMap, MaxPDValue, AvgRange=25, FWHM=4): #turns PD trace into windowed, averaged list of 140 pts; for TraceFormatting2 used AvgRange=1 and FWHM=1
...
#FIX THIS!!!
    def _weich(self,OChan):
        #scope channels and power meters always the same, perhaps shouldn't be part of LOSC class
        # pass instead the one you want, it has look-up config of which scope, which channel, which power meter
        templistQ=self._rch(OChan)
        bkgrdbuffer=int(0.038*len(templistQ))
        bkgrdQ=np.mean(templistQ[:bkgrdbuffer])
        ensampQ=SOME_ENERGY #EG1wYFE1in()[0 for YFE1w or 1 for 1in1w] #EG1w2in() for 2in1w #EG() for 2in2w
        weightQ=ensampQ/np.sum(np.array(templistQ)-bkgrdQ) #for 2in1w:ensampQ[0][OChan-1] #for 2in2w:ensampQ[0][0][OChan-1]
        #or setting leaving weightQ=1 if temppvHead1.get()*temppvHead2.get() == 0, i.e. head wasn't enabled
...
#FIX THIS!!!
    ...
    def psefc10Hz(pwt=0,numIterQ=50,AQQ=0.03,displayPlot=True,reloopPrompt=True):#was B; replaced when LeCroyA fixed
    ...
    def psrefrwvfm(RecipeStrQ,numStepsQ=50,stepSizeQ=0.25,displayPlot=True,reloopPrompt=True):
        ...
        try:#used to make sure shutters re-open even in case of error or KeyboardInterrupt
            ...
            psefc10Hz(pwt=yfegoal,numIterQ=numStepsQ,AQQ=stepSizeQ,displayPlot=displayPlot,reloopPrompt=reloopPrompt)
...
class EnergyMeters:
    def LPLInChamber(printDisplay=False):
        temppvWest=EpicsSignal('MEC:GENTEC:01:CH2:MEAS');tvalW=temppvWest.get();
        temppvEast=EpicsSignal('MEC:GENTEC:01:CH1:MEAS');tvalE=temppvEast.get();
        if printDisplay:
