...
def ref_only(xray_trans=1, xray_num=10, calibrant='', rate=1, visar=False, save=False):
    '''
...
def optical_shot_ABGH(lpl_ener=1.0, timing=0.0e-9, xray_trans=1, msg='', tags_words=['optical', 'sample'], auto_trig=False, auto_charge=False, visar=True):
    '''
    Description: script to shoot the optical laser and time it with the xrays. It automatically push to the elog the laser energy, the timing and the xray SiT transmission.
    IN:
        lpl_ener   : waveplate settings for the lpl energy, decimal value, meaning 1. = 100%, 0.5 = 50%
    ...
    # only selecting channels ABGH
    
    
    # charging process
    ARMonly('ABGH', set_T=lpl_ener)
    time.sleep(5)
    if (auto_charge == True):
        print("Waiting until charging is authorized ...")
        while lpl_charge_status.get()==0:
    ...
    RunNumber = get_run_number(hutch='mec', timeout=10)
    mecl = elog.ELog({'experiment':experimentName}, user='mecopr', pw=pickle.load(open('/reg/neh/operator/mecopr/mecpython/pulseshaping/elogauth.p', 'rb')))
    msg = msg + 'Laser shot ABGH {}: laser energy is at {:.1f} % from max, delay is {:.2f} ns, SiT at {:.1f} %.'.format(msg_log_target, 100.0 * lpl_ener, 1.0e9 * timing, 100.0 * xray_trans)
    mecl.post(msg, run=RunNumber, tags=tags_words)
    # make sure the event sequencer is getting ready for the alignment mode of the VISAR by starting the event sequencer at 1Hz, and no need to touch the trigger as they are ready for this task
    x.start_seq(1)
...
def optical_shot_EFIJ(lpl_ener=1.0, timing=0.0e-9, xray_trans=1, msg='', tags_words=['optical', 'sample'], auto_trig=False, auto_charge=False, visar=True):
    '''
    Description: script to shoot the optical laser and time it with the xrays. It automatically push to the elog the laser energy, the timing and the xray SiT transmission.
    IN:
        lpl_ener   : waveplate settings for the lpl energy, decimal value, meaning 1. = 100%, 0.5 = 50%
    ...
    RunNumber = get_run_number(hutch='mec', timeout=10)
    mecl = elog.ELog({'experiment':experimentName}, user='mecopr', pw=pickle.load(open('/reg/neh/operator/mecopr/mecpython/pulseshaping/elogauth.p', 'rb')))
    msg = msg + 'Laser shot EFIJ {}: laser energy is at {:.1f} % from max, delay is {:.2f} ns, SiT at {:.1f} %.'.format(msg_log_target, 100.0 * lpl_ener, 1.0e9 * timing, 100.0 * xray_trans)
    mecl.post(msg, run=RunNumber, tags=tags_words)
    # make sure the event sequencer is getting ready for the alignment mode of the VISAR by starting the event sequencer at 1Hz, and no need to touch the trigger as they are ready for this task
    x.start_seq(1)
...
def optical_shot_all(lpl_ener=1.0, timing=0.0e-9, xray_trans=1, msg='', tags_words=['optical', 'sample'], auto_trig=False, auto_charge=False, visar=True):
    '''
    Description: script to shoot the optical laser and time it with the xrays. It automatically push to the elog the laser energy, the timing and the xray SiT transmission.
    IN:
        lpl_ener   : waveplate settings for the lpl energy, decimal value, meaning 1. = 100%, 0.5 = 50%
